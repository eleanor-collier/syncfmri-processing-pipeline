...
convert_data() {
  # Get mode for config file (odd or even)
  if [ $((10#$subj_id%2)) -eq 0 ]; then
    mode=even
  else
    mode=odd
  fi
  # Run dcm2bids
  dcm2bids -d "${GET_DATA_HERE}" -p ${subj_id} -s ${session} -c "${GET_CONFIG_HERE}/${session}_${mode}_config.json" -o "${SEND_DATA_HERE}" --forceDcm2niix
}
