...
# Date: August 22, 2018
#
# Create nuisance regressors for resting state ROI functional connectivity analysis.
...
PREP_DIR=$PROJECT_DIR/analysis/restPrep #path to output nuisance regressors
DATA_DIR=$PROJECT_DIR/data/fMRI #path to look for raw data

# Input Volumes
VENTRICLE_VOL=/opt/fsl/5.0.7/data/standard/MNI152_T1_2mm_VentricleMask.nii.gz
...
def_vars() {
  ...
  raw_epi="$DATA_DIR/sub-${subj}/${session}/func/*run-${r}_bold.nii.gz"
  output_dir=${PREP_DIR}/preprocessed/sub-${subj}/rest_run${r}.feat/nuisance
...
setup() {
    tmp_dir=$(mktemp -d --tmpdir rst_prep.XXXXXX)
